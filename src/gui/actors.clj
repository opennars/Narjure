...
(def nodes [{:name :input :px 800 :py -400 :displaysize 10.0 :backcolor gui-color}
            {:name :sentence-parser :px 800 :py -300 :backcolor util-color}
            {:name :task-creator :px 800 :py -150 :backcolor task-color}
            {:name :concept-manager :px 800 :py 150 :backcolor task-color}
            {:name :task-buffer :px 800 :py 0 :backcolor derived-task-color}
            {:name :concepts :px 0 :py 150 :backcolor concept-color}
            {:name :general-inferencer :px 400 :py -150 :backcolor derived-task-color}
            {:name :inference-request-router :px 0 :py -150 :backcolor derived-task-color}
            {:name :inference-request-buffer :px 0 :py 0 :backcolor derived-task-color}
            {:name :operator-executor :px -300 :py 150 :backcolor [255 200 200]}
            #_{:name :task-dispatcher :px 400 :py 150 :backcolor task-color}])

(def edges [{:name ":sentence-msg" :from :sentence-parser :to :task-creator :unidirectional true}
            #_{:name ":task-msg" :from :task-buffer :to :concept-manager :unidirectional true}
            {:name ":task-msg" :from :task-creator :to :task-buffer :unidirectional true}
            {:name ":create-concept-msg" :from :task-buffer :to :concept-manager :unidirectional true}
            {:name ":task-msg" :from :concept-manager :to :concepts :unidirectional true}
            #_{:name ":task-msg" :from :task-dispatcher :to :concepts :unidirectional true}
