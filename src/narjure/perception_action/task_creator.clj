...
(defn sentence-handler
  "Processes a :sentence-msg and generates a task, and an eternal task
   if the sentence is an event, and posts to task-buffer."
  [from [_ sentence]]
  (let [syntactic-complexity (syntactic-complexity (:statement sentence))]
    (when true #_(< syntactic-complexity max-term-complexity)
      (let [new-task (create-new-task
        ...
        (cast! task-buffer [:task-msg new-task])
        (output-task :input new-task)
        (when (event? sentence)
          (when (belief? new-task)
            (reset! lastevent new-task))
          (cast! task-buffer [:task-msg (create-eternal-task new-task)]))))))

(defn derived-sentence-handler
  "processes a :derived-sentence-msg and posts to task-buffer"
  [from [_ sentence]]
