(ns narjure.core
  (:require
    ...
    [narjure.control
     ...
     [inference-request-buffer :refer [inference-request-buffer]]
     [task-buffer :refer [task-buffer]]]
...
(defn inference-tick
  ...
  (cast! (whereis :inference-request-buffer) [:inference-tick-msg]))

(defn system-tick
  "Apply an system tick"
  []
  (cast! (whereis :task-buffer) [:system-time-tick-msg])
...
(def child-specs
  #(list
    ...
    ["1" :permanent 5 5 :sec 100 (task-buffer)]
    ...
    ["3" :permanent 5 5 :sec 100 (inference-request-buffer)]
